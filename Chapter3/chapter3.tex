...
\node[rect,below=of init] (microseg)
{
	\begin{tabular}{@{}p{\llargeur}p{\rightcol}@{}}
		...
		Phase volumetric enthalpy 		& $(\Tj,\wiphi_j) \rightarrow \avg{\rho h}^\phi_j$ \\
	\end{tabular}
};
...
\end{tikzpicture}
\caption{Algorithm for a single temperature to enthalpy (\emph{T2H}) conversion at node $j$.}
...
\node[rect,below=of brent_init] (microseg)
{
	\begin{tabular}{@{}p{\llargeur}p{\rightcol}@{}}
		...
		Phase volumetric enthalpies 	& $(\Tjbrent,\wiphi_j) \rightarrow \crochet{\avg{\rho h}^\phi}^\brent$ \\
	\end{tabular}
};
...
\end{tikzpicture}
\caption{Algorithm for a single enthalpy to temperature (\emph{H2T}) conversion at node $j$.}
...
in the previous iteration, $\Hj^\iter$. The detailed flow chart for the \emph{Hsolver} is given in \cref{fig:flowchart_hsolver}.
...
This solver will be referred to as \emph{Tsolver}. The corresponding flow chart is given in \cref{fig:flowchart_tsolver}.
...
%\begin{figure}[htbp]
%\centering
%	\includegraphics[width=1.5\textwidth]{Chapter3/Graphics/multicomponent/tabulations/planche_tabulations.pdf}
