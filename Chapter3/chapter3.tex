...
are injected in \cref{eq:Navier-Stokes2} then use the output in the coarse scale equations. Further technical details about the method and the equations are found in the PhD work of \citet{hachem_stabilized_2009}. 
...
\end{equation*}
%
where some assumptions have been made:
\begin{enumerate}
\itemsep0em
...
\item	the metal is incompressible, therefore the liquid and solid densities are constant and equal (hence
		$\rholref$ in the inertial term) and the mass balance reduced to $\nabla \cdot \vit =0$
...
\ifthenelse{\boolean{enable_animations}}%
	...
	%\centering
 	 %------------
    		...
    		%\centering
			\includegraphics[height=6cm]{Chapter3/Graphics/freckle_cafe/anim_monograin_upright/img0133.png}%
			\caption{}
  			\end{subfigure}
   			%-----
   			\hspace{10mm}
			...
			%\caption{}
  			\end{subfigure}
   			...
   			\begin{subfigure}[t]{0.35\textwidth}
    		\centering
			\includegraphics[height=6cm]{Chapter3/Graphics/freckle_cafe/anim_monograin_tilted/img0133.png}%
			\caption{}
  			\end{subfigure}
   			%-----
