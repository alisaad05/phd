...
\node[rect, below=of init] (brent_init)
{
	\begin{tabular}{@{}p{\llargeur}p{\rightcol}@{}}
		...
		Iteration &	$\brentzero$ \\
		Temperature & $T_j^\brentzero = \Tjtminus$
	\end{tabular}
};
...
\node[rect,below=of brent_init] (microseg)
{
	\begin{tabular}{@{}p{\llargeur}p{\rightcol}@{}}
		...
		Phase mass enthalpies 			& $(\Tjbrent,\wiphi_j) \rightarrow \crochet{\hphi_j}^\brent$ \\
		Phase densities 				& $(\Tjbrent,\wiphi_j) \rightarrow \crochet{\rphi_j}^\brent$ \\
	\end{tabular}
};
...
\end{multline}
%------------
where $H=\rho h$ is a simplified notation denoting the average volumetric enthalpy, used to derive the weak formulation. 
...
\node[rect,below=of solve] (microseg)
{
	...
	Tabulation (\emph{H2T})	&	$ H^\iterplus_j \rightarrow \Tkplus_j$ 	\quad (\cref{fig:algorithm_h2t}) \\
	Update				& 	$\dTdHjkplus=f(\Tjkplus,H_j^\iterplus)$ 	\quad (\cref{eq:finite_difference_Hsolver})\\
	\end{tabu}
};
...
\node[rect,below=of solve] (microseg)
{
	\begin{tabular}{@{}p{\llargeur}p{\rlargeur}@{}}
	\textbf{Microsegregation} \\
	Tabulation (\emph{T2H}) 	&	$ \Tkplus_j \rightarrow H_j^\iterplus$ 		\quad (\cref{fig:algorithm_t2h})  \\
	Update				& 	$\dHdTjkplus=f(\Tjkplus,H_j^\iterplus)$ 			\quad (\cref{eq:finite_difference_Tsolver})\\
	\end{tabular}
};
...
General microsegregation models coupled with thermodynamic tabulations (e.g. \citep{tourret_multiple_2011}) for multicomponent alloys is still missing. 




\input{Chapter3/recap3} % resume francais
