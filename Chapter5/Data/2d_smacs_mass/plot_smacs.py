...
fn = "global_smacs"
z1 = np.genfromtxt(fn+".txt", delimiter='\t', skip_header=0, skip_footer=1000, names=True , usecols=("Temps","msoluteM", "mM","vA","vM") )
mSize = 6
mFreq = 1000 #Mark plot every x number of points
mTransp = 0.85
c1 = 'k'
c2 = 'g'
c3 = 'm'
...
ax.plot(z1['Temps'], metalmass_pct , marker = "s", label=r"$m^M_\%$", color=gray1, markersize=mSize, markevery=mFreq, alpha=mTransp, lw=lw, ls=":")
...
ax2 = ax.twinx()
ax2.plot(z1['Temps'], vA_pct , marker = marker.next(), label=r"$v^A_\%$", color="b", markersize=mSize, markevery=mFreq, alpha=mTransp, lw=1, ls="--" )
ax2.plot(z1['Temps'], vM_pct , marker = marker.next(), label=r"$v^M_\%$", color="r", markersize=mSize, markevery=mFreq, alpha=mTransp, lw=1, ls="--")



ax.set_xlabel('Time (s)')
ax.set_ylabel("Relative mass change ($\%$)")
ax2.set_ylabel("Relative volume change ($\%$)")
