...
First, we look to the results in \cref{fig:1200s_unmask}. The original average composition field obtained by \emph{grid A} is almost
free of steep composition gradients except for one segregated channel rising from the bottom by the action of thermal convection. 
...
\centering
  ...
  \caption{Solid fraction on \emph{grid B}}
    \label{fig:1200s_gs}
  \end{subfigure}
\caption{Snapshot of the average composition field shown on (a) \emph{grid A} where the mesh gets coarser in fully solidified regions near the right side while
...
metal is obtained by the successive transport operations performed in (c) a restricted area (\yellow{yellow} color) based on the nodal values of the liquid fraction.}
