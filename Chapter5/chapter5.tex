...
Nevertheless, it should be mentionned that the mapping resolution plays an important role in the accurary of thermodyamic conversions.
...
combinations to scan while computing equilibirum.
\Cref{table:mapping_stat} gives a clear idea of the computation times and tabulation size for each alloy. It is clear that computation time is increasing according
to the number of tabulated lines for \emph{b1Bin}, \emph{b1Tern} and \emph{b1Quat}.
...
resulting file is three time smaller than the original tabulation file. Suprisingly, the computation time for \emph{b1QuatLite} shows no significant acceleration 
...
is called. However, \cref{table:mapping_stat} reveals that the multivariable interpolation overhead is even more important, resulting in longer computation times.
...
\end{table}
%--------------------------------
% time taken at 20 s running 20 cores: OK for Quat and QuatLite TODO: relaunch Bin and Tern
...
are presented in \cref{fig:final_segreg_ternquat}. First, we compare the carbon segregation as it is the common species among the presented alloys.
The first striking difference is a remarkable positive mesosegregation at the chill contact of the \emph{b1Quat} sample, while for \emph{b1Tern}, the positive
segregation intensity is less, and for \emph{b1Bin} it is even less. As we explained earlier, this positive mesosegregation taking place at the beginning
of solidification
