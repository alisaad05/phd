...
(cf. \cref{fig:real_ingot_stage_b}). As a direct result of the inward feeding flow, the ingot free surface with the air
...
to a different overall volume ($V^s<V^l$) once solidification is complete, as confirm the following mass conservation equations, from initial (LHS term) to final (RHS term) state:
%------------
\begin{subequations}
\begin{align}
& \rhol V^l = \rhos V^s  \\ 
...
Thermal shrinkage is very important to apprehend in steel casting, as the temperature decrease usually exceeds a \SI{1000}{\udegC}, thus causing substantial density variations. 
...
set method make it very attractive to apply it to macroscopic surface tracking (in contrast to microscopic interface tracking, 
...
It means that no gas phase may appear during the process.
...
Furthermore, with the introduction of shrinkage, an increase in global density of the metallic alloy means that a gas phase should enter the domain to replace the shrunk volume.
...
physical interfaces between the several phases. In our applications, we are particularly 
...
It means that no subsequent deformation or contraction/dilatation of the solid phase ($\rhos=\text{constant}$) may occur and therefore $\vsolair$ reduces to vector zero.
...
In reality, between the metal and the air, two interfaces exist: the liquid-air ($\liqair$) and solid-air ($\solair$) interfaces, 
...
In this case, the Air-Metal boundary is defined by the $\liqair$ interface. The latter is driven by solidification shrinkage 
and therefore by the real microscopic velocity of the interdendritic liquid, $\vl$. According to \citet{dantzig_solidification_2009},
...
reaches the free liquid surface, creating hence these distinct interfaces: 
a first interface separating interdendritic liquid from the air, and a second interface that separates the dendrites also from the air. 
...
until the transition zone (intersection of both domains, identified by $\heaviside^{tr}$) and and the mushy zone (solid+liquid) overlap, as shown in the numerical row of stage 3 in \cref{fig:1dalsi7_interface_stages}.
At this point, both interfaces define the Air-Metal boundary. Although it is necessary to track the solid-air interface, the present work limited to considering the liquid-air interface as
fully defining the boundary between domains. Tracking another interface adds complexity as an additional tracking method should be used like a new level set with distinct numerical properties.
This assumption will have some influence regarding the overall simulation performance, as some errors are induced.
...
% caption
{Schematic describing the physical (first row) and numerical (second row) process of moving an interface between air and metal domains, in three intermediate stages of 
...
require attention even before investigating the feeding flow created by solidification. As a demonstration, \cref{fig:1dalsi7_velocity_dt} shows the effect of different
...
\comment{PARAGRAPH MOVED TO INTORIDUCTION, MAKE NECESSARY CORRECTIONS}
% However in reality, 
...
%-------------------
 \begin{axis}
  [	name=whatever,
	...
	ylabel=  $\frac{\avg{w}-w_0}{w_0}$ (\%), %Relative mesosegregation,
  ...
  \addplot table [x expr=1000*\thisrow{arc_length}, y expr=100*(\thisrow{Concentration}-0.105)/0.105]   {Chapter5/Data/3d_segregation/BinTransformationPath.csv};
...
\end{tikzpicture}
%-------
\caption{Segregation profile in percent with respect to the nominal composition, along the vertical revolution axis of the solidified sample.}
...
{\tabulinesep=1.0mm \begin{tabu}{lccccc}
\tabucline[1pt]{-}
\textbf{Alloy} & \textbf{Nb solutes} & \textbf{Nb phases}  & \textbf{Tabulation lines}  & \textbf{Size (MB)} & \textbf{Computation time (s)}  \\\tabucline[1pt]{-}
%-----------------------------
      \emph{b1Bin}            & 1                       &  4                  &  \num{185010}     &    \num{4.37}   &  \num{59461.5}  \\   %78003.9
      \emph{b1Tern}           & 2                       &  5                  &  \num{220250}     &    \num{10.18}  &  \num{62644.4}  \\    
      \emph{b1Quat}           & 3                       &  5                  &  \num{1101250}    &    \num{66.89}  &  \num{85476.7}  \\
      \emph{b1QuatLite}       & 3                       &  5                  &  \num{326014}     &    \num{20.93}  &  \num{84104.8}  \\\tabucline[1pt]{-}
...
\end{tikzpicture}
%-------
\caption{Relative mesosegregation profiles as functions of the distance from the chill, plotted for (a) carbon (b) silicon and (c) manganese elements
