...
By assuming a fixed solid phase ($\vs=\vec{0}$), i.e. a constant density for the solid phase without any transport of this phase, the average velocity in the metal reduces only to liquid's average velocity. Therefore, we can write:
%------------
\begin{align}
\label{eq:vitmetal}
...
In this situation, $\rhos=\rhol= \avg{\rho}= \avg{\rho}$ and the term $\partial \avg{\rho}/\partial t$ therefore vanishes.
...
Darcy dissipative term in the metal's Navier-Stokes system makes it more difficult to formulate a single monolithic equation. This is discussed
after writing the equations of the monolithic model.

\subsubsection{Mass and momentum conservation}

The system's velocity, $\vitf$, is given by an arithmetic mixing between each domain's relative average fluid velocity, 
...
alloy's shrinking volume. 

As no volume change was considered for the air, $\mix{\Phi}$ is present only 
