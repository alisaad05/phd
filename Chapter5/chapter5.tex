...
The nominal composition for this alloy is \bin{Fe}{0.105}{C}. In order to obtain accurate segregation results, a fine resolution mapping was performed
...
%-------------------
 \begin{axis}
  [	name=whatever,
	...
	ylabel= Relative mesosegregation (\%), %\\$(\avg{w}-\avg{w_0})/\avg{w_0}$ (\%),
	xlabel=Distance from the chill (mm),
	%xticklabel style={/pgf/number format/fixed},
	...
	\addplot table [x expr=\thisrow{arc_length}*1000, y expr=100*(\thisrow{Concentration_M}-0.105)/0.105] {Chapter5/Data/3d_segregation/z_axis.txt};
  ...
  [	name=whatever,
	...
	\addplot table [x expr=\thisrow{arc_length}*1000, y=FractionBCC_c] {Chapter5/Data/3d_segregation/z_axis.txt};
	\addplot table [x expr=\thisrow{arc_length}*1000, y=FractionFCC_c] {Chapter5/Data/3d_segregation/z_axis.txt};
	\addplot table [x expr=\thisrow{arc_length}*1000, y=FractionCEM_c] {Chapter5/Data/3d_segregation/z_axis.txt};
	%\draw[thick, dashed, black, opacity=0.5] (axis cs:\pgfkeysvalueof{/pgfplots/xmin},0.0) -- (axis cs:\pgfkeysvalueof{/pgfplots/xmax},0.0);
	\legend{BCC,FCC,CEM}
  	\end{axis}
\end{tikzpicture}
%-------
\caption{Segregation profile in percent with respect to the nominal composition, along the vertical axis of the droplet.}
...
We have presented earlier in this book how we predict macrosegregation in a Fe-C-Cr ternary alloy using the thermodynamic tabulation approach.
In this section, we simulate reduced-gravity solidification of the \emph{b1} alloy, where the latter is considered as a ternary and then as a quaternary (cf \cref{table:texus_b1})
alloy. For simplicity, we refer to them respectively by \emph{b1T} and \emph{b1Q}.
The aim is to show on the one hand, we are able to scale our model to fit any multicomponent alloy (even if only assuming full or partial equilibrium), while
on the other hand study how solidification paths vary by adding additional components, hence changing the shrinkage kinetics.
By introducing additional chemical species, silicon for \emph{b1T}	then silicon and magnesium \emph{b1Q}, a new carbide phase may appear, the M$_7$C$_3$.
 
After 15 seconds of chill contact, the \emph{b1T} and \emph{b1Q} alloys have more volume fraction of liquid than the previous binary computation as shown in 
\cref{fig:multicomp_liquid}, meaning that
solidification in slower. The reason could be attributed to the different solidification paths created by the system composition, but also it is possible that phase enthalpies
are higher than for the binary system. Regardless of its cause, slower solidification means that the final predicted shape is different, as shown earlier in the parametric study.
The latter showed that slower cooling rates result in more elongated droplet shapes due to weight force, whereas shrinkage would tend to do the opposite.
...
{Misc/dummy.pdf}
% caption
{Snapshots showing the volume fraction of liquid remaining in each sample of the binary, ternary and quaternary \emph{b1} alloy.}
% label
\label{fig:multicomp_liquid}
\end{figureth}
%--------------
% {Chapter5/Graphics/3d/nonbin/phase_distrib.pdf}
