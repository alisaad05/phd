...
As the simulation starts, we use the \emph{Remesh4} adaptive technique to maintain accurate predictions for velocity and interface transport.
...
Although this is possible with the chosen remeshing technique, it is more difficult to maintain a fine mesh size throughout the simulation, 
and some information may be lost when coarser elements are obtained by remeshing. Therefore, to avoid such unwanted effects, we use another uniform
...
The strategy consists of checking the liquid fraction of each node in \emph{grid A}, if its value is located between 30\% and 70\%, then we consider that is a 
a region of interest, since the flow has not been yet fully stopped due to the low permeablility. We consequently transport the average composition field exclusively 
for these nodes, keeping for all other nodes their respective average composition values. 
