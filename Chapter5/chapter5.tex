...
% ********************************************
\subsection{Constant volume solidification}
% ********************************************
When no solidification shrinkage is considered, the metal volume remains constant. It is therefore interesting to compare the 
segregation patterns, first when making this hypothesis then comparing with a case of solidification shrinkage.
As this case was previously introduced in chapter 3, for the \emph{Tsolver}'s validation, we present only the results. 
The boundary conditions considered by \citet{caroozani} assumed that the metal-air surface is adiabatic with an imposed zero velocity
vector. When using the level set method, it is possible to compute the fluid velocity in the viciniy of the interface, thus imposing directly the velocity in this area is avoided. 
However, the effect of changing the upper boundary condition on the flow and segregation patterns is important. 
Therefore, in \cref{fig:smacs_noLS_2800,fig:smacs_noLS_7000}, we compare at 2800 s and 7000 s a no-slip condition on the top boundary to an imposed zero normal velocity but with a free-slip tangential component. 
...
\label{fig:smacs_noLS_7000}
\end{figure}
%------------------

In \cref{fig:smacs_noLS_2800} showing an early stage of solidification, the top no-slip wall acts a brake for the moving flow, 
deviating it downwards. A smaller magnitude flow is then created near the upper boundary, with less heat and chemical species 
transport, allowing a fast solification rate compared to the slip boundary case. In the latter, the flow 
