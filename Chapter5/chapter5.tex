...
still not perfect as some key input parameters are still missing in the model, namely the real gravity acceleration onboard the parabolic flight (which should
...
% caption
{Comparison of experimental and numerical shrinkage profiles, compared to their respective initial shapes. A vector image processing algorithm is used
to extract the droplet outlines from the experimental images. The experimental deformation shows a slight non-symmetricity which cannot be reproduced by the numerical model.}
% label
...
the sample at low temperatures to achieve solid-state transformation. In our simulations, this transformation cannot be seen as the cooling time is limited
to primary and peritectic solidification.
...
{1.0}
%path 
{Chapter5/Graphics/3d/simu_exp.pdf}
% caption
