...
\paragraph{Non-monolithic strategy:}
...
of the reduced transport for the higher blocking fraction. However, it should be pointed out that the differences between 300 s and 800 s are not important because the permeability predicted by the Carman-Kozeny model, falls to zero quickly for liquid fractions less than about 60\%. 
...
\tabucline[1pt]{-}
\textbf{Configuration} & \textbf{Diffusion in air [\si{\udiffusivity}]} & \textbf{Advection in air [\si{\uvelocity}]} & \textbf{Composition reinitialisation} \\\tabucline[1pt]{-}
%-----------------------------
MD	&	\num{1.5e-12} &  $\vitf$  	&  -	\\ 
MDA	&	\num{1.5e-12} &  $\vec{0}$  &  -	\\ 
...
\Cref{fig:1dalsi7_gs_w} shows only the first 1.5 cm of the solidifying sample, therefore a complete segregation profile is plotted along the sample length in \cref{fig:1dalsi7_wVSlength}.
...
We cannot much of differences when comparing mass analysis for all three techniques, as shown by the dotted lines in \cref{fig:mass_species_alsi7}.
...
\centering
  ...
  \end{subfigure}
   %------------
\caption{Final macrosegregation patterns obtained experimentally by solidifying a \bin{Sn}{3}{Pb} alloy at (a) \SI{0.02}{\uCR} and (b) \SI{0.04}{\uCR}. 
...
All mass and volume data are plotted in \cref{fig:smacs_mass_volume}. The main reason for grouping all plots in a single
...
% this plot wont fucking work because of the fucking size limit memory in latex, even with shell escaping doesnt work
% %--------------------------------------
% \shorthandoff{;:?!} %http://tex.stackexchange.com/questions/74860/unexpected-clash-between-babel-and-pgf-spy
% \begin{figure}[htbp]
% \centering
...
% 	legend cell align=left,
% ]
%   \addplot table [x=Temps, y=mMpct]   			{Chapter5/Data/2d_smacs_mass/smacs_mass.txt};
%   \addplot table [x=Temps, y=msoluteM_pct]  	{Chapter5/Data/2d_smacs_mass/smacs_mass.txt};
  
%   \legend{m^M_\%,m^\text{sp}_\%}  	
  
%   \end{axis}
%   	%------------------
% \end{tikzpicture}
% %-------
% \caption{Mass variation plots for the metal and its chemical species}
% \label{fig:mass_species_smacs}
% \end{figure}
% %--------------------------------------
% \shorthandon{;:?!} %http://tex.stackexchange.com/questions/74860/unexpected-clash-between-babel-and-pgf-spy
...
%-------------------
 \begin{axis}
  [	name=whatever,
	...
	scaled ticks=true,
	ylabel style={align=center},
	ylabel=  Relative content $\frac{\mix{\avg{w}}-w_0}{w_0}$ (\%), %Relative mesosegregation,
