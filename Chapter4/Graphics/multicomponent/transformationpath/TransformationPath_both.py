...
# WITH MACRO
data2 = np.genfromtxt('capteursTP_macro/Capteur1.txt', delimiter=None, skip_header=0, skip_footer=0, names=True , usecols=("Temps","FractionLiq","FractionBCC","FractionFCC","FractionM7C3","FractionCementite") )
...
x, y1, y2, y3, y4, y5 = data2['Temps'], data2['FractionLiq'], data2['FractionBCC'], data2['FractionFCC'], data2['FractionM7C3'], data2['FractionCementite']

ax2.plot(x,y1, color= c1, linestyle= ls, marker= m1, markersize=ms, linewidth=width1,   markevery=NbMarker, alpha=alphaT, label="Liquid"  )
ax2.plot(x, y2, color= c2, linestyle= ls, marker= m2, markersize=ms, linewidth=width1,   markevery=NbMarker, alpha=alphaT, label="BCC"  )
ax2.plot(x, y3, color= c3, linestyle= ls, marker= m3, markersize=ms, linewidth=width1,   markevery=NbMarker, alpha=alphaT, label="FCC"  )
ax2.plot(x, y4, color= c4, linestyle= ls, marker= m4, markersize=ms, linewidth=width1,   markevery=NbMarker, alpha=alphaT, label="M7C3"  )
ax2.plot(x, y5, color= c5, linestyle= ls, marker= m5, markersize=ms, linewidth=width1,   markevery=NbMarker, alpha=alphaT, label="CEM"  )
...
# ax2.set_title('Transformation Paths (without macrosegregation)')
...
ax1.legend([Liquid, BCC, FCC, CARB, CEM, pos1,pos4],['LIQ', 'BCC', 'FCC', 'M$_7$C$_3$', 'CEM','Bottom', 'Top'], fancybox=True, shadow=True, bbox_to_anchor=[0.5, 0.5], 
           loc='center', ncol=2)
