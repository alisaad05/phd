...
\subsubsection{Solidification shrinkage effect on temperature}
...
\subsubsection{Inverse segregation}
...
solute is progressively depleted in the remaining liquid, causing negative macrosegregation in the last solidified nodes of the metal, 
as clearly shown in \cref{fig:1dalsi7_wVSlength}.
...
\begin{tikzpicture}
  ...
  [	name=one,
	...
	\draw[thin, dashed, black, opacity=0.5] (axis cs:\pgfkeysvalueof{/pgfplots/xmin},7) -- (axis cs:\pgfkeysvalueof{/pgfplots/xmax},7);
  	\end{axis}
  	%-------------------
 
\end{tikzpicture}
%-------
\caption{Plot of the average composition as function of length, along a vertical line passing through the center of the sample.}
...
solid grains sedimentation and volume shrinkage.
...
\caption{Final macrosegregation patterns in (a) \bin{Sn}{5}{Zn} and (b) \bin{Sn}{5}{Pb} castings, with a visible sign of volume shrinkage at the top. 
		The rectangular grid on both alloys is used to measure concentration at control points located at each rectangle's centre \citep{hebditch_observations_1974}.} 
\label{fig:HH_exp}
\end{figure}
%------------------
...
\subsection{Previous experimental and numerical studies}
