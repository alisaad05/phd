...
%-----------------
%
\subsubsection{Shrinkage without macrosegregation}
%----------------------------------------------------
...
100 seconds of cooling while for the last frame, the frame was taken after only 1 second of cooling, thus it is crossed to show non-convergence.
...
Although no solidification has yet started at 100 s, a two-dimensional flow is observed around the interface, while no driving forces exist.
...
to the properties transition across the interface, namely density and dynamic viscosity. It is also noted that the interface position
is not modified by the neighbouring currents, that reach a maximum magnitude of \SI{e-4}{\uvelocity}. Furthermore, this fact is confirmed by a computation 
having the same parameters as the second case in \cref{fig:1dalsi7_velocity_dt}, but giving the air subdomain the metal's properties, making it 
...
It is noted that the time step in the latter simulation is changed to 0.1 s, as the air behaves mechanically like a fluid metal, 
therefore no transitions are seen by the interface.
...
{Misc/dummy.pdf}
% caption
{Comparison computation showing the effect of taking equal properties across the level set interface.}
% label
\label{fig:1dalsi7_equalprops}
\end{figureth}
%--------------

\subsubsection{Shrinkage with macrosegregation}
...
%--
\subsection{3D application: TEXUS steel}
