...
These spurious interface shape changes are mainly caused by the presence of a mushy zone in contact with the diffuse interface, affecting the level set transport 
and thus causing problems regarding mass conservation.
In contrast, \cref{fig:1dalsi7_differentprops} shows more viable results as far as the level set transport is concerned.
From 200 s to 800 s, the local flow instability discussed in \cref{fig:1dalsi7_velocity_dt} is sustained, even until after solidification is complete.
However, in regions of 100\% metal and 100\% air the computed velocity is nearly the same order of magnitude as predicted in the \cref{fig:1dalsi7_equalprops} case.
We notice a recirculating air flow in the vicinity of the interface as no metal shrinkage may further occur once solidification is complete, thus air flows freely in and out 
of the upper boundary with a very low magnitude ($\sim$ \SI{e-7}{\uvelocity}), while impinging on the air-metal surface.

The "real" air simulation ran for 23.3 hours of CPU time on 6 processors while the "artificial" air simulation lasted only 2 hours, running on the same number of
processors.
%-----------------------------------------
\begin{figure}[htbp]
\centering
...
$l^M_f= \brac{1-\beta_{SS}} l^M_0 =$  0.09286 \si{\metre}. However, this equation is very useful in more complex
...
\begin{axis}
[	name=artificial,
	title=(a) "Artificial" air - $\Delta t$=0.01 s,
	scale only axis,
	table/col sep=tab,
	smooth, %ybar
	%stack plots=y,
...
\begin{axis}
[	name=real,
	title=(b) "Real" air - $\Delta t$=0.01 s,
...
\end{tikzpicture}
%================================================
\caption{UPDATE CAPTION Variation of the metal's mass versus solidification time in different cases with respect to the liquid blocking fraction varies from between two limiting
...
There is no surprise of how poor mass conservation is with the level set method. However, the mass variation
that we see in the previous plots is more related to a physical problem: \textbf{at which velocity does the interface move?}
In our simulations, we systemically considered the Navier-Stokes solution $\vitf$ (equal to $\vit= \gl \vl$ in the metal subdomain),
that serves to transport the metal-air interface. However in reality (depicted by the first row in \cref{fig:1dalsi7_interface_stages}, 
between the metal and the air two interfaces exist: the liquid-air ($\liqair$) and solid-air ($\solair$) interfaces, and these interfaces become more defined as solidification proceeds.
...
the mushy zone delimited by dendrite tips, reaches the free liquid surface, creating hence two distinct interfaces: a first interface separating
...
The first factor is ubiquitous in any solidification process. In the present work, we remind that the solid phase is assumed fixed and rigid, therefore we consider dendrites
to be undeformable during their growth. Unfortunately, this assumption is contradictory with our current situation where the metal is shrinking upon solidification, 
until dendrite tips become close to the metal-air interface (overlapping of diffuse interface region and mushy zone, shown in lower right corner of \cref{fig:1dalsi7_interface_stages}).
...
to determine a adequate transport velocity field. 
%--------------
\begin{figureth}
% textwidth 
{1.0}
