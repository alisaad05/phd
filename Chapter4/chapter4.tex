...
If we consider the liquid phase, these variations are given by:
%------------
%\begin{subequations}
\begin{align}
\label{eq:}
& \tempup{\avg{\psi^l}} = \avg{{\tempup{\psi^l}}} + \frac{1}{\rev} \integral{l-a}{\psi^l \vliqair_n}{\Gamma}
+ \frac{1}{\rev} \integral{l-s}{\psi^l \vliqsol_n}{\Gamma} \\
& \nabvec{\avg{\psi^l}} = \avg{\nabvec \psi^l} -\vec{n}^{\liqsol} \delta\brac{x-x^{\liqsol}}
\end{align}
%\end{subequations}
%------------
%
\comment{link with LS transport equation by looking into \citep{rappaz_numerical_2003} p. 113
where the heaviside is the same as the indicator function.}
%
%======================
\section{Metal FE model}
%--------------------------------------------------
%
...
The metal domain consists of two phases, solid and liquid, which have unequal densities that vary with temperature.
However, as we consider in the current project that the solid is static, then $\vec{v^s}=\vec{0}$, hence only the liquid's
density may vary while the solid has a constant density. With these assumptions, one may write:
%
\begin{align}
