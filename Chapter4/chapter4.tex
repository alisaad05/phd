...
\subsubsection{Mass and momentum conservation}
%
Considering that the system's velocity, $\vec{v}$, is given by an arithmetic mixing between each domain's average velocity, we write:
%---
\begin{align}
\label{eq:monolithic_mass_1}
& \nabla \cdot \vec{v} = \nabla \cdot \brac{\heavisideM \vM + \heavisideA \vA} \\
\label{eq:monolithic_mass_2}
& \nabla \cdot \vec{v} = \heavisideM \nabla \cdot \vM + \heavisideA \nabla \cdot \vA + \nabvec \heavisideM \cdot \brac{\vM - \vA}\\
...
any volume variation of the metal domain, will be followed by an air inflow any volume loss. The third and last term in the same equation 
...
& \nabla \cdot \vec{v} = \heavisideM \brac{-\frac{1}{\rhol} \brac{\tempup{\avg{\rho}^M }+ \vit \cdot  \nabvec \rhol}}
\end{align}
%---
The momentum balance looks like the one derived for the metal, but using level set mixed properties and bearing in mind 
that the Darcy term has a special treatment explained in the next section:
%------
\begin{equation}
\label{eq:monolithic_momentum}
   \left\{
   \begin{aligned}
      & \mix{\rho} \brac{\tempup{ \vec{v} } + \frac{1}{\gf} \nabvec \cdot \brac{\vec{v} \times \vec{v}}} = \\
	  &- \gf\nabvec p - 2 \mix{\mu} \nabvec \cdot \brac{\nabmat \vec{v}+ \nabmattransp \vec{v}}
	  - \gf \mix{\mu} \Kmodif^{-1} \vec{v} + \mix{\rho g} \gravity\\ \\
      & \nabla \cdot \vec{v} = \heavisideM \brac{-\frac{1}{\rhol} \brac{\tempup{\avg{\rho}^M }+ \vit \cdot  \nabvec \rhol}}
    \end{aligned}
    \right.
\end{equation}
%------------
EXPLAIN $\mix{\rho g} \gravity$ 
%---
\subsubsection{Energy conservation}
%
\subsubsection{Mass and momentum conservation}
...
\end{enumerate}
%
We define a fluid fraction, $\gf$, as an arithmetic mixing between liquid and air fractions across the interface:
%
\begin{align}
...
\end{align}
%---------------
then from the fluid fraction, we deduce a modified permeability, $\Kmodif$. 
...
of the interface while being transported.
