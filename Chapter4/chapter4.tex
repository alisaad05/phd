...
\begin{table}[htbp]
\centering
\caption{Nominal composition (\si{\ucomposition}) of the \emph{b1} steel}
\label{table:texus_b1}
{\tabulinesep=1.0mm \begin{tabu}{lcccccc}
\tabucline[1pt]{-}
\textbf{Alloy} & \textbf{C} & \textbf{Mn} & \textbf{Si} & \textbf{Al} & \textbf{S} & \textbf{P} \\\tabucline[1pt]{-}
%-----------------------------
\emph{b1}				&	0.105 	& 		0.636	&	0.268	&	\num{0.0067} 	&		0.009		&	0.0189		\\
Binary \emph{b1}		&	0.105 	&		-		&	-		&			-		&		-			&		-		\\
Ternary \emph{b1}		&	0.105 	& 		0.636	&	-		&			-		&		-			&	 	-		\\		
Quaternary \emph{b1}	&	0.105 	& 		0.636	&	0.268 	&			-		&		-			& 		- 		\\\tabucline[1pt]{-}
%-----------------------------
\end{tabu}}
\end{table}
%--------------------------------
...
The interesting point about performing the same reduced-gravity simulation while varying the alloy from binary to quaternary, is the effect 
on solidification paths caused by segregation, but also try to seek for a possible interaction between the solidification paths 
and the final droplet shape, whereas the shrinkage profile is directly related to the solid fraction and its evolution with time.

\subsection{Texus binary alloy}
We first with the binary alloy, given previously in \cref{table:texus_b1}.
