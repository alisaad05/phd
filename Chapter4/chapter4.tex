...
\begin{itemize}
\itemsep0em
\item \gray{Gray} curve - case G0: pure diffusion solidification with $\rhol=\rhos$ (no level set) used previously in chapter 2 for validation;
...
Finally, \cref{fig:macro_effect} compares cases B and R, both with unequal phase densities but only predicting macrosegregation in the latter case. 
Differences are not striking, as temperatures
along the metal sample are the same. We spot however a difference at the 10 cm probe, 
...
However, the average composition reaches a positive peak of $\avg{w}$=\bin{Al}{7.5}{Si} (CHANGE VALUE ACCORDING TO PLOT) with the initiation and progression of the eutectic front.
...
\Cref{fig:1dalsi7_gs_w} shows only the first 1.5 cm of the soldifiying sample, therefore a complete segregation profile is plotted along the sample length in \cref{fig:1dalsi7_wVSlength}a,
...
%[spy using outlines={circle, magnification=3, connect spies}]
  ...
  [	name=one,
	...
	\addplot table [x=arc_length, y=Concentration] {Chapter4/Data/1d_alsi7_composition/ModifyDiffSoluteAir.csv}; %\closedcycle ;
...
%-------------------
 \begin{axis}
  [	name=two,
	...
	\addplot table [x=arc_length, y=Concentration] {Chapter4/Data/1d_alsi7_composition/ModifyVitLiq.csv}; %\closedcycle ;
	%\addplot[thick, samples=50, smooth,magenta, name path=ls] coordinates {(0.09268,\pgfkeysvalueof{/pgfplots/ymin})(0.09268,\pgfkeysvalueof{/pgfplots/ymax})};
	\draw[thick, magenta,  opacity=0.5] (axis cs:0.0926,\pgfkeysvalueof{/pgfplots/ymin}) -- (axis cs:0.0926,\pgfkeysvalueof{/pgfplots/ymax});
	\draw[thick, dashed, black, opacity=0.5] (axis cs:\pgfkeysvalueof{/pgfplots/xmin},7) -- (axis cs:\pgfkeysvalueof{/pgfplots/xmax},7);
  	\end{axis}
%-------------------
 \begin{axis}
  [	name=three,
	...
	\addplot table [x=arc_length, y=Concentration] {Chapter4/Data/1d_alsi7_composition/ModifyVitLiq_ModifyDiffSoluteAir.csv}; %\closedcycle ;
	%\addplot table [x=arc_length, y=Concentration] {Chapter4/Data/1d_alsi7_composition/ModifyVitLiq_ModifyDiffSoluteAir_NoSolute.csv}; %\closedcycle ;
	%\addplot[thick, samples=50, smooth,magenta, name path=ls] coordinates {(0.09268,\pgfkeysvalueof{/pgfplots/ymin})(0.09268,\pgfkeysvalueof{/pgfplots/ymax})};
	\draw[thick, magenta,  opacity=0.5] (axis cs:0.0926,\pgfkeysvalueof{/pgfplots/ymin}) -- (axis cs:0.0926,\pgfkeysvalueof{/pgfplots/ymax});
	\draw[thick, dashed, black, opacity=0.5] (axis cs:\pgfkeysvalueof{/pgfplots/xmin},7) -- (axis cs:\pgfkeysvalueof{/pgfplots/xmax},7);
  	\end{axis}
...
\end{tikzpicture}
%-------
\caption{Plot of the average composition as function of length, along a vertical line passing through the center of the sample at 1000 s, with 
		(a) low solute solute diffusion in the air but unmodified solute advection (minimum composition reaches \SI{2.2}{\ucomposition} Si but not shown), 
...
{1.5}
%path 
{Chapter4/Data/1d_alsi7_arc_compositions/compositions_vs_length.pdf}
% caption
...
composition results with different Sn-Pb and Pb-Sn alloys. In the current section, we are interested in the latter experiment.
...
\centering
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    \centering
	\includegraphics[width=\textwidth]{Chapter4/Graphics/2d/HH1.png}
	\caption{}
    \label{fig:HH1}
  \end{subfigure}
  %------------------------------
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    \centering
	\includegraphics[width=\textwidth]{Chapter4/Graphics/2d/HH2.png}
	\caption{}
    \label{fig:HH2}
  \end{subfigure}
   %------------
\caption{Final macrosegregation patterns in (a) \bin{Sn}{5}{Zn} and (b) \bin{Sn}{5}{Pb} castings, with a visible sign of volume shrinkage at the top. 
...
{My caption}
% label
\label{fig:Experimental setup used onboard TEXUS flights, showing the initial alloy sample, the cage and the positiong coil. The setup
...
from above it. An axial pyrometer measures the sample temperature during the process. Also, a high-speed camera records the solidification process, useful
to measure the front growth speed. Each parabola cycle lasts for 50 s, offering an effective low gravity ($\norm{\gravity}\approx$\SI{e-1}{\uacceleration}) for about 20 s.
...
but for simplicity we will refer to the latter as being the TEXUS experiment. The main difference with respect to parabolic flight experiments,
...
{0.5}
%path 
{Chapter4/Graphics/3d/camera_end.pdf}
% caption
{Camera image of the fully solidified droplet with a deformed shape, after 10 s.}
% label
...
Furthermore, the substrate is implicitly taken into account via a boundary condition. This is sufficient in the 
current context, because we are only interested in the energy transfer from the droplet to the substrate. The latter is hence modeled 
Fourier condition with $\Text$=\SI{25}{\udegC} and an effective exchange coefficient $\hext$ of \SI{2500}{\uhconvec}.
The $\hext$ coefficient's value has been determined by running multiple simulations with different values in the aim
of achieving the same solidification time as in the experiment. The steel sample is a pseudo-spherical droplet of \SI{3}{\milli \metre}-radius,
with a planar surface (diameter of \SI{1.5}{\centi \metre} at its south pole, where the contact is initiated. The alloy is immersed in a gas (argon)
medium, such that both subdomains form together 1/4 of a cylinder having \SI{9999}{\milli \metre} in radius and  \SI{9999}{\milli \metre} in height.

As for the boundary conditions, \cref{fig:texus_bc} shows that the droplet-substrate 
contact is considered without any slip, since this area solidifies in the first place without further fluid motion, as shows FIGURE EXP.
...
This problem has been reported by \citep{basset_simulation_2006}, showing a limitation in the imposed boundary conditions between Navier-Stokes solver and
level set transport. Therefore, we limit these instabilities by imposing a no-slip condition, thus allowing the argon to flow in the computational
domain through the generatrix. The pressure condition is left free for all boundaries, as the experiment does not provide any pressure measurement 
of the confined argon.
%--------------
\begin{figureth}
% textwidth 
{0.4}
...
{Misc/dummy.pdf}
% caption
{3D views showing the different boundary conditions used for reduced-gravity simulations.}
% label
\label{fig:texus_bc}
\end{figureth}
%--------------
...
{0.3}
%path 
{Chapter4/Graphics/3d/transport_instability.png}
% caption
{2D view showing the two interfaces: the green line is the initial metal-gas interface, while the blue line represents
the transported zero-level of the distance, showing a transport instability at the top.}
% label
\label{fig:transport_instability}
\end{figureth}
%--------------

\subsubsection{Nominal composition}
Various steel grades were considered in the CCEMLCC projet. Each grade was assigned to a specific experiment. 
We limit our study to the steel aigned for TEXUS missions, the grade is designated as "b1" alloy. Its ominal 
composition is given in TABLETABLE
...
\addplot table [x=time, y=position] {Chapter4/Data/3d/olddroplet_frontposition_vs_time.txt};
\addplot table [x=time, y expr=\thisrow{position}/1000] {Chapter4/Data/3d/EXP_texus2009_frontposition_vs_time.txt};
\addplot table [x=time, y expr=\thisrow{position}/1000] {Chapter4/Data/3d/exp_tempus2014_frontposition_vs_time.txt};

\legend{Simulation, TEXUS, TEMPUS}
\end{axis}
\end{tikzpicture}
\caption{Position of solidification front versus time for the binary alloy simulation compared to the experimental findings of the TEXUS-46 flight in 2009 and TEMPUS 2014 measurements \citep{gandin_project_2014}.}
...
{1.0}
%path 
{Chapter4/Graphics/3d/simu_exp.pdf}
% caption
{Comparison between experimental (TEMPUS) and numerical (TEXUS) results showing the initial profile in both cases in \green{green}, while the final deformed profile
  is the \blue{blue} line obtained experimentally, the \red{red} line predicted numerically. The direction of the gravitational fields points downwards, depicted
...
\subsection{Texus ternary alloy}
\subsection{Texus quaternary alloy}
