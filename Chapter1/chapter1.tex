...
of course depends on the equations being discretised and solved on the FE grid. The easiest meshing solution one can 
choose is to create homogeneous and isotropic grid (\cref{fig:mesh_iso_homo_unstruct}), while respecting some constraints regarding the time step (temporal
...
or Courant–Friedrichs–Lewy (CFL) condition in advective equations. In such a case, errors due to interpolation are minimised,
which guaranties good results but with an expensive time cost. 
...
equation. Although this is an interesting alternative, it is less powerful than anisotropic meshing. In the latter (\cref{fig:mesh_aniso_hetero_unstruct}),
elements adapt to a physical quantity, such as enthalpy or velocity, 
reducing the elemental length in the direction where the gradient is higher, while stretching the element in the orthogonal direction. This allows more accurate 
resolution with less elements than needed by isotropic meshing. Moreover, this type of meshing is well adapted to the context of this thesis 
as it allows local mesh refinement around the mobile interface over time, as well as regions undergoing solidification. These regions
are indeed important to track as microsegregation starts in the mushy zone, i.e. between the liquidus and solidus temperatures given by a local solute
...
\centering
  ...
  \begin{subfigure}[t]{0.25\textwidth}
    \centering
	\includegraphics[width=\textwidth]{Chapter1/Graphics/mesh_iso_hetero_unstruct.png}
	\caption{}
  ...
  \end{subfigure}
   ...
   \begin{subfigure}[t]{0.25\textwidth}
    \centering
	\includegraphics[width=\textwidth]{Chapter1/Graphics/mesh_aniso_hetero_unstruct.png}
	\caption{}
...
Since mesh adaptation involves a mathematical notions, readers interested in the basics are referred to the following references:
...
done at CEMEF. These techniques rely on metric tensors, briefly explained in the next section, and belong to
\emph{a posteriori} error estimators category. The metric information is then passed to a mesh generation tool, \emph{MTC},
which is based on an iterative procedure of local topology optimisations.
%--------------------------------------
\section*{To do ?}
