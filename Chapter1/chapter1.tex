...
\comment{in the three following sections, maybe I should add a small schematic next to each each equation to visualize concentration profiles}
%------------
%
%***********************************************
...
%----
\comment{I should adopt only one symbol for the differential inside the surface integrals $\mathrm{d} A$ or $\mathrm{d} \Gamma$}
\begin{subequations}
\begin{align}
...
The position, denoted $x$, is hence updated as follows:
%------
\begin{align}
\label{eq:lagrangian}
& x^{(t+1)} = x^{(t)} + \vec{v} \Delta t
\end{align}
%------------
...
% caption
{Schematic of the interface (thick black line) of a rising air bubble in water. The other contours represent isovalues
of the distance function around and inside the interface contour.}
% label
...
\heaviside= 
\begin{cases}
	0  & \text{ if } \levelsetx < -\epsilon \\
    1  & \text{ if } \levelsetx > \epsilon \\  
    \frac{1}{2} \brac{1+ \frac{\levelsetx}{\epsilon} + \frac{1}{\epsilon}\sin \brac{\frac{\pi \levelsetx}{\epsilon }} } & \text{ if } - \epsilon \leq \levelsetx \leq \epsilon
\end{cases}
\end{align}
%-----------
...
\comment{add two sentences about dirac and curvature, as properties that can be readily obtained from the level set}
...
method, this work gives an insight of the mixture possibities and their effect on a pure thermal diffusion.
...
velocity field as input. The problem can be simply written as:
%------
\begin{align}
...
%
%
\subsubsection{Transport formulation}
The finite element method gives the fully discretised weak form of \cref{eq:transport_strong} by using a convenient set of test functions $\levelsettest$:
%-----------
\begin{align}
...
\end{align}
%------------
If we opt for a forward finite difference for time discretisation and the following nodal discretisation in space, assuming $N$ the number
of nodes in any simplex:
%-----------
\begin{align}
\label{eq:transport_weak2}
&  \levelset = \sum_N  N_j \levelset_j
\end{align}
%------------
then \cref{eq:transport_weak1} can be recast as follows:
\comment{continue weak form discretisation}
%-----------
\begin{align}
\label{eq:transport_weak3}
&  \frac{1}{\dt} \levelset_j \integral{\Omega}{N_i N_j }{\Omega} + \levelset_j \integral{\Omega}{\vec{v} \cdot \nabvec N_j }{\Omega}= 0
\end{align}
%------------
%
%
...
\end{align}
%------------
The closer this $L^2$-norm to one, the more regular the level set. An irregular distance function cannot
accurately capture the physical interface and thus results in numerical errors, because of wrong distance information. 
...
water. The importance of this well studied case \citep{hysing_quantitative_2009} 
