...
The \emph{Dirac} delta-function is also an important property to convert surface integrals to volume terms, which could
...
The Dirac delta-function, plotted in \cref{fig:heaviside_dirac} along with the Heaviside function within an interface thickness 
of $\crochet{-\varepsilon;+\varepsilon}$, is derived from the latter as follows:
%--- --------------
\begin{align}
\label{eq:dirac}
...
\end{align}
%-------------
The Heaviside and Dirac functions can be readily processed to obtain other geometric properties from the level set,
...
\text{interface surface area}: A &= \integral{\Omega}{\dirac \norm{\nabvec \levelset}}{\Omega} \\
%
\label{eq:levelset_volume}
\text{inner volume}: V &= \integral{\Omega}{\heaviside}{\Omega}
%
\end{align}
%-----------
...
Note that the peak of the Dirac delta-function depends on the interface thickness to ensure a unity integral of the delta function over $\Omega$. }
...
The main idea is very simple: reconstruct the distance function over $\Omega$ ar a subset of $\Omega$, by computing the minimum distance between each 
the mesh node and the interface. It means that, for any point $\vec{x} \in \Omega$, the following constraint should be 
satisfied \citet{osher_signed_2003}:
%------
\begin{align}
...
Moreover, the authors compare the total time required by a Hamilton-Jacobi reinitalisation,  direct reinitialisation
and the latter using k-d trees, all on the same 2D and 3D test cases. They ultimately demonstrate the great 
performance of direct reinitialisation when used with k-d trees algorithm, hence we adopt their implementation in the present work.
%
%--------------------------------------
\section*{To do ?}
