%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%           Page de garde              %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Dorian Depriester (dorian.depriester@mines-paristech.fr), 2014
% Ali Saad 			(ali.saad@mines-paristech.fr), 			2015

\makeatletter
\def\@ecole{école}
\newcommand{\ecole}[1]{
  \def\@ecole{#1}
}

\def\@specialite{Spécialité}
\newcommand{\specialite}[1]{
  \def\@specialite{#1}
}

\def\@ED{École Doctorale}
\newcommand{\ED}[1]{
  \def\@ED{#1}
}

\def\@doctorat{Doctorat}
\newcommand{\doctorat}[1]{
  \def\@doctorat{#1}
}

\def\@adresse{Adresse}
\newcommand{\adresse}[1]{
  \def\@adresse{#1}
}

\def\@directeur{directeur}
\newcommand{\directeur}[1]{
  \def\@directeur{#1}
}

\def\@encadrant{encadrant}
\newcommand{\encadrant}[1]{
  \def\@encadrant{#1}
}
\def\@jurya{}{}{}
\newcommand{\jurya}[3]{
  \def\@jurya{#1,	& \small{#2}	& \small{#3}\\}
}
\def\@juryb{}{}{}
\newcommand{\juryb}[3]{
  \def\@juryb{#1, & \small{#2}  & \small{#3}\\}
}
\def\@juryc{}{}{}
\newcommand{\juryc}[3]{
  \def\@juryc{#1, & \small{#2}  & \small{#3}\\}
}
\def\@juryd{}{}{}
\newcommand{\juryd}[3]{
  \def\@juryd{#1, & \small{#2}  & \small{#3}\\}
}
\def\@jurye{}{}{}
\newcommand{\jurye}[3]{
  \def\@jurye{#1, & \small{#2}  & \small{#3}\\}
}
\def\@juryf{}{}{}
\newcommand{\juryf}[3]{
  \def\@juryf{#1, & \small{#2}  & \small{#3}\\}
}
\def\@juryg{}{}{}
\newcommand{\juryg}[3]{
  \def\@juryg{#1, & \small{#2}  & \small{#3}\\}
}
\def\@juryh{}{}{}
\newcommand{\juryh}[3]{
  \def\@juryh{#1, & \small{#2}  & \small{#3}\\}
}
\def\@juryi{}{}{}
\newcommand{\juryi}[3]{
  \def\@juryi{#1, & \small{#2}  & \small{#3}\\}
}
\makeatother
%---------------------------------------------------------------------------------------------------------------------------

\newcommand\BackgroundPic{%
	\put(0,0){%
		\parbox[b][\paperheight]{\paperwidth}{%
			\includegraphics[height=1.0\paperheight]{Frontmatter/logo_paristech_mines_border_etiquette.pdf} %
			\vfill
		}
	}
}

%\newcommand\EtiquetteThese{%
	% \put(0,0){%
		% \parbox[t][\paperheight]{\paperwidth}{%
			% \hfill
			% \colorbox{blue}{		
				% \begin{minipage}[b]{3em}
					% \centering \large \textcolor{white}
					% {\textbf{\textsf{\vspace{0.6cm}\\T\\\vspace{0.4cm}H\vspace{0.5cm}\\E\\\vspace{0.4cm}S\\\vspace{0.4cm}E\\\vspace{1cm}}}}
				% \end{minipage} %È
			% }
		% }
	% }
% }

%---------------------------------------------------------------------------------------------------------------------------

\makeatletter
\newcommand{\mytitlepage}{
\newgeometry{top=4cm, bottom=1cm, left=0.5cm, right=1.5cm}
\AddToShipoutPicture*{\BackgroundPic}
%\AddToShipoutPicture*{\EtiquetteThese}
  \begin{titlepage}
  \centering
      {\fontsize{14}{12} \@ED}\\
    \vspace{1cm}
      {\fontsize{18}{12}
      	{\bfseries \@doctorat}\\
    \vspace{0.5cm}
      	\fontsize{16}{12} \textbf{T H È S E}}\\
    \vspace{0.5cm}
   		{\fontsize{11}{12} \bfseries pour obtenir le grade de docteur délivré par}\\
    \vspace{1cm}
    	{\fontsize{18}{12} \bfseries \@ecole}\\
    \vspace{0.5cm}
    	{\fontsize{14}{12} {\bfseries Spécialité doctorale ``\@specialite''}}\\
    \vspace{1.5cm}
    	\fontsize{11}{12} \textit{présentée et soutenue publiquement par}\\
    \vspace{0.5cm}
    	{ \fontsize{16}{12} {\bfseries \@author}} \\
    \vspace{0.5cm}
    	le \@date \\
    \vfill
       {\fontsize{18}{22} \color[rgb]{0,0,1} \bfseries{\@title}} \\
    \vfill
               
        \begin{tabular}{lll}
        \fontsize{11}{12}

    & Co-directeur de thèse: & {\bfseries \@encadrant}\\
    & Co-directeur de thèse: & {\bfseries \@directeur}
		\end{tabular}
	
    \vfill
	\begin{tabular}{>{\bfseries}lll}
		\large Jury\\
		\@jurya
		\@juryb
		\@juryc
		\@juryd
		\@jurye
		\@juryf
		\@juryg
		\@juryh
		\@juryi
	\end{tabular}
	\vfill
	
	\@adresse
  \end{titlepage}

\restoregeometry  
}