...
& \brac{R_i}^\iter = \brac{R_i}^\iterminus + \brac{\dRdH}^\iterminus_{ij} \Delta \Hj^\iterminus + \order\brac{\Hj^2}
\end{align}
%------------
...
& \brac{\dRdH}^\iterminus_{ij} \brac{\Hj^\iter - \Hj^\iterminus} = -R_i^\iterminus
\end{align}
%------------
where $\dRdH$ is a global tangent matrix yielding the variations of the residual with respect to the enthalpy 
in the last iteration, $\Hj^\iterminus$. If \cref{eq:discretise_residual_local} is considered, then the contribution of an element $\OhmE$ writes:
%-----------------
\begin{align}
\label{eq:dRdH}
& \brac{\dRdH}^{E\iterminus}_{ij} 
= \Mij^E 
+ \underbrace{\Aij^E \brac{\dTdH}^{\iterminus}_{j}}_{\nosum}
+ \underbrace{\brac{\Kija^E + \Kijb^E} \brac{\dTdH}^{\iterminus}_{j}}_{\nosum}
\end{align}
%------------
\Cref{eq:dRdH} is the core of the enthalpy-based solver. The resolution of \cref{eq:Hsolver_residual_linear} 
then yields a new estimate of the vector of nodal enthalpies $H^\iter$, which are the only unknowns to be solved for. 
...
& \brac{\dRdT}^\iterminus_{ij} \brac{\Tj^\iter - \Tj^\iterminus} = -R_i^\iterminus
\end{align}
%------------
where $\dRdT$ is a global tangent matrix yielding the variations of the residual with respect to temperature $\Tj^\iterminus$ at the previous iteration. 
...
\begin{align}
\label{eq:dRdT}
& \brac{\dRdT}^{E\iterminus}_{ij} 
= \Mij^E 
+ \underbrace{\Aij^E \brac{\dHdT}^{\iterminus}_{j}}_{\nosum}
+ \underbrace{\brac{\Kija^E + \Kijb^E} \brac{\dHdT}^{\iterminus}_{j}}_{\nosum}
\end{align}
%------------
...
then yields a new estimate of the vector of nodal temperatures $T^\iter$, which are the only unknowns to be solved for. 
...
At a nonlinear iteration $\iter$, this term is written using a finite difference:
%-----------------
\begin{align}
\label{eq:finite_difference_Tsolver}
& \textbf{Tsolver} \qquad \brac{\dHdT}^{\iterminus}_{j} = \frac{\rhoh_j^\iter - \rhoh_j^\iterminus}{T_j^\iter - T_j^\iterminus} \\
\label{eq:finite_difference_Hsolver}
& \textbf{Hsolver} \qquad \brac{\dTdH}^{\iterminus}_{j} = \frac{T_j^\iter - T_j^\iterminus}{\rhoh_j^\iter - \rhoh_j^\iterminus}
\end{align}
%------------
...
is re-evaluated with the newly determined temperature $\Tj^\iter$ and enthalpy $\Hj^\iter$ so \cref{eq:discretise_residual_global} rewrites:
%-----------------
\begin{align}
\begin{split}
\label{eq:discretise_residual_global_new}
& \brac{R_i}^\iter(T) = \Mij \brac{H(\Tj^\iter) - \Hjtminus} + \Aij \Tj^\iter + \brac{\Kija + \Kijb}\Tj^\iter -\Fi - \Si \\
& i,j:1 \rightarrow \text{NbGlob}
\end{split}
\end{align}
%------------
The norm of the current residual, $\norm{R^\iter}$, is compared to a fixed small 
